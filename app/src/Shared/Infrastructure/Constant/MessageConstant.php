<?php

namespace App\Shared\Infrastructure\Constant;

final class MessageConstant
{
    const SUCCESS_MESSAGE_TO_CREATE = 'Los datos han sido creados.';
    const SUCCESS_MESSAGE_TO_UPDATE = 'Los datos han sido actualizados.';
    const INVALID_TOKEN_CSFR_MESSAGE = 'El token csrf no es válido.';
    const UNEXPECTED_ERROR_HAS_OCCURRED_ON_DELETE = 'Ha sucedido algo inesperado al intentar eliminar, por favor intente nuevamente.';
    const UNEXPECTED_ERROR_HAS_OCCURRED = 'Ha sucedido algo inesperado, por favor intente nuevamente.';
}
